...
import { number, number, z } from "zod";
  ...
  const content = `
    <div>
      ...
      ${order.items.map(item => `
        <p class="bg-red-500">${amount} x ${pizzas.find(pizza => pizza.id === item.id)!.name} <button id="remove">X</button></p>
      `)}
